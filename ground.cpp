...
static void buildGroundTile(TFT_eSprite &tile, int grassHeight) {
  ...
  // Clamp so we don't draw outside the tile.
  if (grassHeight > TILE) {
    grassHeight = TILE;
  }
  ...
  if (grassHeight >= 2) {
    int highlightY1 = 1;
    int highlightY2 = grassHeight > 3 ? 2 : grassHeight - 1;
    for (int x = 1; x < TILE-1; x += 3) {
      tile.drawPixel(x, highlightY1, grassLite);
      tile.drawPixel(x+1, highlightY2, grassLite);
    }
  }
}

void groundInit() {
  buildGroundTile(groundTileTop, 5);
  buildGroundTile(groundTileTransition, 3);
  buildGroundTile(groundTileDirt, 0);
}
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    for (int c = 0; c < cols; ++c) {
      ...
      if (r == 0) {
        ...
        groundTileTransition.pushSprite(x, y);
      } else {
